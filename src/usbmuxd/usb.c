...
static int usb_device_add(libusb_device* dev)
{
	...
	libusb_set_configuration(handle,current_config);
	...
	usbmuxd_log(LL_WARNING, "WWW  current_config %d  NumConfigurations %d", current_config , devdesc.bNumConfigurations);
	...
	usbmuxd_log(LL_WARNING, "WWW devdesc.iSerialNumber %d  res:%d",devdesc.iSerialNumber,res);
	...
	if((res = libusb_get_string_descriptor_ascii(handle, devdesc.iSerialNumber, (uint8_t *)usbdev->serial, 256)) <= 0) {
		...
		#else
		const char * default_serial = "iphone_device";
		usbmuxd_log(LL_WARNING, "WWW reset device serial number to :%s",default_serial);
		
		//设置默认的名称，计算名称长度
		sprintf(usbdev->serial,"%s",default_serial);
		res = strlen(default_serial);
		#endif
	}
