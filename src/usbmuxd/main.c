...
static const char *socket_path = "/dev/usbmuxd";
static const char *lockfile = "/dev/usbmuxd.pid";
...
static int create_socket(void) {
	...
	return socket_local_server(socket_path,ANDROID_SOCKET_NAMESPACE_ABSTRACT,SOCK_STREAM);
...
#ifndef HAVE_PPOLL
static int ppoll_ext(struct pollfd *fds, nfds_t nfds, const struct timespec *timeout, const sigset_t *sigmask)
{
	int ready;
	sigset_t origmask;
...
static int main_loop(int listenfd)
{
	...
	while(!should_exit) {
		...
		cnt = ppoll_ext(pollfds.fds, pollfds.count, &tspec, &empty_sigset);
